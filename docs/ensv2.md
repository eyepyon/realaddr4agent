...
policy v2のERC1155残高確認はregisterだけに限定し、期待registryからownerへの増加1と、receipt blockの実登録tokenIdを照合する。既存stateにv1のhashがある場合、`ENS_PARENT_PREVIOUS_WRAPPER_POLICY_FILE`の内容hashを一致させ、既存4 contract pinを変えない追加更新だけを許す。finality待ちは登録失敗とは区別し、再送しない。
