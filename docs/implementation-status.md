...
| T-07 LeaseRegistry | コントラクト実装・ローカルEVM検証済み、library登録確認済み | role・slot/version・取消・区画再利用の制約を実装し、Foundry 12件が通過。Ethereum Sepolia接続walletのread-only確認とMultiBaas library definitionの登録・version read-back・ABI/creation bytecode一致を確認。MultiBaas UI deployは正しいconstructor値でもtransaction不足エラーとなり、wallet署名画面が開かない。unsigned deploy API応答はHTTP 200でtransaction data・sender・zero valueを返すがsubmitted:false。署名・送信はしていない。UI原因は未特定。実deploy/transaction、role設定、contract read/write/events、DB/outbox接続は未完了 |
...
登録用artifact exporterはcompiler設定、ABI、未解決link、各sourceのKeccak-256とcompiler metadataの一致を検査し、ABI・creation/runtime bytecode・SHA-256 manifestを未追跡領域へ生成する。最終CIの3ファイルはローカル生成物とbyte単位で一致し、通常CIも通過した。Ethereum Sepoliaのread-only接続確認とMultiBaas library definitionの登録・version read-back・ABI/creation bytecode一致を確認した。これはlibrary登録の証拠であり、on-chain deploy/transactionではない。UIのDeploy Contractではconstructor値を設定後も「Missing the transaction to continue」となり、wallet promptが開かなかった。診断したunsigned deploy APIはchain ID一致、登録artifactとのABI/bytecode一致、HTTP 200、正しいtransaction data・sender・zero valueを返し、`submitted:false`だった。署名・送信は呼び出しておらず、UI原因は未特定である。ローカルMetaMask署名helperを実装した。模擬providerによる重点6件（別chain/wallet拒否、固定creation transaction、重複・不明結果の保持、明示拒否後の再試行、receipt照合、保存失敗時の送信拒否）が通過した。loopbackのHTTP smokeではHost/Origin/token、任意file拒否、試行状態保持、拒否後の再試行、別portからの同時起動拒否を確認した。実起動とブラウザー表示も確認済みで、これらの確認で実ウォレットからの署名・送信は行っていない。Sepolia deploy、transaction/hash、role設定、runtime code照合、contract read/write/events、永続cursor/reorg回復、DBのランダムchain keyとoutbox publisherの統合は残件。T-07全体の完了チェックは付けない。
