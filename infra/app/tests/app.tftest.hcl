...
run "foundation_closed" {
  command = plan
  assert {
    condition     = length(google_service_account.app) == 4 && length(google_cloud_run_v2_service.app) == 0 && length(google_cloud_scheduler_job.sweep) == 0 && length(google_project_iam_member.firestore) == 0 && length(google_cloud_run_v2_service_iam_member.web_public) == 0 && !var.dispatch_enabled
