...
Keep external account identifiers, local paths, URLs, secrets, deployment identities, and provenance out of repository documents, source comments, examples, and commit messages. Record only this project's behavior and configuration contract.
...
- The GCP project is shared. Use `RESOURCE_PREFIX=realaddr-event` for dedicated resource names and `FIRESTORE_COLLECTION_PREFIX=realaddr_event_` for every logical collection, including admin, guard, and outbox collections. A Firestore name prefix is not an IAM boundary. Use the `(default)` database only as a shared reference when needed; do not import it into this app's Terraform state or manage its database, rules, indexes, project IAM, project APIs, or budgets wholesale. Create separate web, worker, Cloud Tasks invoker, and Scheduler invoker service accounts for this app; do not use default service accounts or service account keys as fallbacks. Select the deploy service account through `DEPLOY_SERVICE_ACCOUNT` in protected configuration. Its lifecycle is outside this app's Terraform state: do not create, import, or delete the account or replace its IAM policy. Inventory its owner, bindings, and effective permissions; add only reviewed grants on this app's resources and a narrowly scoped WIF impersonation member while preserving other bindings. Do not assume its effective permissions are limited to this app. Create this app's own repository-restricted WIF pool/provider. Google-managed service agents remain project-level identities; scope their grants to this app's resources where possible. Never overwrite or delete shared resources. Index changes may target only this app's prefixed collection groups. Confirm live ownership and inventory before using any apparently unused name; test runtime IAM and Firestore client Rules as separate gates.
