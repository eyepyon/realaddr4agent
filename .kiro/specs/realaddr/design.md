...
読み取り照合の初期実装では、buildingにbuildingKey、leaseにleaseKey/holderSalt/holderCommitmentと最新確定支払いを指すregistryPaymentOrderIdを保存する。支払いpointerは購入・更新の確定transactionで更新し、identityはclaim付き初回prepareで原子的に確保して更新時にも保持する。outboxのregistryChangeへ公開可能な期待値を固定し、同じ確定blockでのRPC・MultiBaas照合後にregistryEvidenceをoutboxとleaseへ保存する。証跡はchain ID、registry address、block番号/hash、照合対象を含み、claim/versionを再検査してからsyncedへ更新する。古いjobはsuperseded、成功したadmin pending projectionは削除する。署名・送信および取消の業務遷移はこの読み取りhandlerに含めない。

registryの区画占有は、未取消かつ `expiresAt > block.timestamp` の現在契約に限る。期限切れ・取消後はcontract上は別leaseKeyで同じ区画を記録でき、旧記録は保持する。ただしv1の業務処理ではR-01.5に従い他テナントへ再利用せず、DBとpublisherがこの制約を維持する。旧leaseの同version再送は区画占有を書き換えず、旧leaseの取消も現在の占有が自分の場合だけ解放する。旧leaseのpaid revivalを記録する場合は、別の有効な現在契約が占有していれば拒否する。遅延したoutboxの照合用に過去の正の期限も記録できるが、現在の占有を置換しない。これはDBのslot所有・paid状態の検査を代替しない。
