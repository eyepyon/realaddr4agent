...
const formalTerms = termsVersion === CURRENT_TERMS_VERSION;
  ...
  return <main className="app-page" id="main-content">
    ...
    <section className="app-content"><p className="eyebrow">利用者ポータル</p><h1>{session ? "ご自身の契約" : "walletで接続"}</h1>
      {!session && <div className="login-card"><p>接続したwalletの署名でAgent APIへ認証します。秘密鍵を入力する欄はありません。署名はAPI sessionの取得だけに使い、決済や契約更新は実行しません。</p>
        ...
        <section className="notice terms-summary" aria-labelledby="terms-title"><h2 id="terms-title">{formalTerms ? "利用規約の要点" : "開発認証条件の要点"}（{termsVersion || "未設定"}）</h2><ul><li>この署名はAgent APIへの認証だけに使い、購入・更新・送金を実行しません。</li><li>住所利用は30日単位の契約です。ENS名は別途選択する任意の初回追加購入です。</li><li>Agent APIは人間が入力した完全な転送先住所を表示せず、郵便転送の有効化は人間の明示承認を必要とします。</li><li>ハッカソン版では郵便物の受領・発送や送料決済を行いません。World認証は法的本人確認を意味しません。</li><li>利用できる販売環境、決済、拠点は運用者による有効化が必要です。画面の表示だけで利用可能とは限りません。</li></ul><p><a href="/faq">サービスの範囲とFAQを見る</a></p><p><a href="/terms">利用規約 version 1の全文を見る</a></p>{!formalTerms && <p>現在は開発認証条件への確認です。この開発用署名を正式な利用規約version 1への同意として扱いません。</p>}</section>
        <label className="consent-row"><input type="checkbox" checked={consented} onChange={(event) => { setConsented(event.target.checked); setChallenge(null); }} /> <span>{formalTerms ? <>利用規約 version 1（{CURRENT_TERMS_VERSION}）の全文を読み、同意したうえでwallet challengeを確認して署名します。</> : <>上記の開発認証条件（{termsVersion || "未設定"}）を読み、内容を理解したうえでwallet challengeを確認して署名します。正式規約への同意ではありません。</>}</span></label>
