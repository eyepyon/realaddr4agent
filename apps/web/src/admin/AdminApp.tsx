...
export function AdminApp() {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main" id="main-content">
      <div className="admin-topbar"><div className="admin-heading"><p className="eyebrow">運用コンソール</p><h1>{pageLabel}</h1><p>{session.displayName} · operator · session期限 {labelValue("expiresAt", session.expiresAt)}</p></div><div className="admin-actions"><button className="admin-button" disabled={busy || mutationBusy} onClick={refresh}>更新</button><button className="admin-button" onClick={() => void request("/v1/admin/session", { method: "DELETE", headers: { "X-CSRF-Token": session.csrfToken, "Idempotency-Key": crypto.randomUUID() } }).then(() => window.location.reload()).catch((e) => setNotice({ kind: "error", text: publicError(e) }))}>ログアウト</button></div></div>
