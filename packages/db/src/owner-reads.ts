...
export interface PublicLease { id: string; locationId: string; floor: number; displayAddress: string; status: 'active' | 'expired' | 'suspended' | 'revoked'; startsAt: string; expiresAt: string; version: number; chain: {status: 'pending' | 'synced'; network: string}; mail: {status: 'disabled' | 'suspended'; destinationConfigured: boolean; physicalForwardingAvailable: false}; ens: PublicEnsStatus }
