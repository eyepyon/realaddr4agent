...
const contractName = process.argv[2] ?? 'LeaseRegistry';
if (!['LeaseRegistry', 'RealAddrNameController'].includes(contractName)) throw new Error('Unsupported contract');
...
const safePath = key => {
  ...
  assert(key === `src/${contractName}.sol` || key.startsWith('node_modules/@openzeppelin/contracts/') || key.startsWith('@openzeppelin/contracts/'), 'Unexpected compiler source');
...
try {
  ...
  const artifact = JSON.parse(await readFile(resolve(contracts, `out/${contractName}.sol/${contractName}.json`), 'utf8'));
  ...
  assert(target && Object.keys(target).length === 1 && target[`src/${contractName}.sol`] === contractName, 'Unexpected compilation target');
  ...
  if (contractName === 'LeaseRegistry') assert(functions.get('getLease(bytes32)')?.stateMutability === 'view' && functions.has('recordLease(bytes32,bytes32,uint16,bytes32,uint64,uint64)') && functions.has('revokeLease(bytes32,uint64)'), 'Expected lease functions missing');
  else assert(functions.get('getBinding(bytes32)')?.stateMutability === 'view' && functions.has('bindLease(tuple)') && functions.has('syncLease(bytes32,uint64)') && functions.has('disableName(bytes32,uint64)'), 'Expected controller functions missing');
  const constructor = artifact.abi.find(entry => entry.type === 'constructor');
  if (contractName === 'LeaseRegistry') assert(constructor?.inputs?.length === 2 && constructor.inputs[0].name === 'admin' && constructor.inputs[1].name === 'writer' && constructor.inputs.every(input => input.type === 'address'), 'Unexpected constructor');
  else assert(constructor?.inputs?.length === 8 && constructor.inputs.slice(0, 7).every(input => input.type === 'address') && constructor.inputs[7].type === 'string', 'Unexpected constructor');
  const bytes = field => {
  ...
  const manifest = { contract: contractName, status: 'local_artifact_only', chainId: 11155111, compiler: metadata.compiler.version, optimizer: { enabled: true, runs: 200 }, evmVersion: 'cancun', openZeppelin: '5.4.0', constructor: constructor.inputs.map(({ name, type }) => ({ name, type })), hashes: { algorithm: 'SHA-256', creationBytecode: sha256(creation), runtimeBytecode: sha256(runtime), abiFile: sha256(abiText) }, sources };
  const destination = resolve(contracts, 'dist');
  await mkdir(destination, { recursive: true });
  await writeFile(resolve(destination, `${contractName}.json`), json(exported), 'utf8');
  await writeFile(resolve(destination, `${contractName}.abi.json`), abiText, 'utf8');
  await writeFile(resolve(destination, contractName === 'LeaseRegistry' ? 'deployment-manifest.json' : `${contractName}.manifest.json`), json(manifest), 'utf8');
  process.stdout.write(`${contractName} local artifacts exported; no deployment performed.\n`);
} catch (error) {
