...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === 'GET' && request.url === '/plan') return send(200, JSON.stringify({ manifest, manifestHash, token }));
    ...
    if (request.method === 'POST' && request.url === '/state') {
      ...
      const operation = writeTail.then(async () => {
        ...
        await writeFile(temporary, JSON.stringify({ manifestHash, revision: revision + 1, state: next }, null, 2) + '\n', { flag: 'wx', encoding: 'utf8' });
...
server.listen(0, '127.0.0.1', () => { origin = `http://127.0.0.1:${server.address().port}`; console.log(JSON.stringify({ origin, manifestHash, environment: 'testnet' })); });
